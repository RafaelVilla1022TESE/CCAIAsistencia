...
/// <summary>
/// Lista de usuarios con búsqueda, alta, edición, eliminación y exportación (en memoria).
/// </summary>
public class UserListView : Form
{
    ...
    public UserListView(BindingList<User> users)
    {
        _users = users;
    ...
    private void InitializeComponent()
    {
        ...
        _grid.Columns.Add(new DataGridViewTextBoxColumn { DataPropertyName = nameof(User.DisplayName), HeaderText = "Nombre", Width = 220 });
    ...
    private void _btnAddCommonButtons()
    {
        _btnNew = new Button
        {
            Text = "Nuevo",
    ...
    {
        using var form = new UserForm(_users, user);
    ...
    private void ExportCsv()
    {
        ...
        sb.AppendLine("Usuario,Nombre,Contrasena,Roles");
        foreach (var u in _filtered)
        {
            sb.AppendLine($"{Escape(u.Username)},{Escape(u.DisplayName)},{Escape(u.Password)},{Escape(string.Join(";", u.Roles))}");
        }
